...
export function HomePage({
  ...
  useEffect(() => {
    if (!worksOverride) {
      void getWorks().then((works) => setFeatured(works.slice(0, 3))).catch(() => undefined);
    }
  }, [worksOverride]);
  ...
  const currentWorks = worksOverride ?? featured;
    ...
    <>
      ...
      <section className="section">
        <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
          ...
          {editMode ? (
            <span className="meta">作品封面可在“作品管理”里右键替换。</span>
          ) : (
