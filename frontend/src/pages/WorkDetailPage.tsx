...
export function WorkDetailPage(): JSX.Element {
  ...
  const [work, setWork] = useState<Work | null>(() => sampleWorks.find((item) => item.id === workId) ?? null);
        ...
        <div className="work-detail-main stack">
          {gallery.length ? (
            <article className="panel stack work-gallery-panel">
              <img src={gallery[activeImage] ?? resolveWorkCoverUrl(work.coverUrl)} alt={`${work.title} 详情图 ${activeImage + 1}`} className="work-detail-hero-image" />
              {gallery.length > 1 ? (
