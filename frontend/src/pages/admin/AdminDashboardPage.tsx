...
export function AdminDashboardPage(): JSX.Element {
  ...
  async function persistSettingsDraft(nextSettings: SiteSettings): Promise<void> {
    ...
    setSocialLinksText(serializeSocialLinks(updated.socialLinks));
    setUiTextJson(JSON.stringify(updated.uiText, null, 2));
  }
  ...
  async function saveWork(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      if (editingWorkId) {
        ...
        setWorks((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
        setWorkDraft(updated);
        setStatus("作品已更新。");
      } else {
        const created = await createAdminWork(payload);
        setWorks((prev) => [created, ...prev]);
        setEditingWorkId(created.id);
        setWorkDraft(created);
        setStatus("新作品已创建。");
      }
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "保存失败。");
  ...
  async function handleSettingsImageUpload(field: SettingsImageField, url: string): Promise<void> {
    ...
    try {
      ...
      setStatus(field === "bannerImageUrl" ? "横幅图已更新。" : "头像图已更新。");
    } catch (error) {
  ...
  async function handleWorkCoverUpload(workId: string, url: string): Promise<void> {
    ...
    try {
      ...
      setStatus(`已更新作品封面：${updated.title}`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "封面保存失败。");
    }
  }
    ...
    <main className="admin-shell">
      <div className="admin-layout">
        <aside className="admin-sidebar" aria-label="后台导航">
          ...
          <p className="meta">现在首页横幅文案可直接改，作品支持长文案、图集和交互亮点编辑。</p>
